...
/**
 * A test suit for the StockRegistryActor class.
 */
public class StockRegistryActorTest extends JUnitSuite {
  ...
  @Test
  public void testHandleEmptyGetStocks() {
    new TestKit(system) {{
      final Props props = Props.create(StockRegistryActor.class);
      final ActorRef subject = system.actorOf(props);
      final TestKit probe = new TestKit(system);
      
      within(duration("1 second"), () -> {
        subject.tell(new StockRegistryMessages.GetStocks(), getRef());
        expectMsgClass(Stocks.class);
  ...
  @Test
  public void testHandleNonEmptyGetStocks() {
    new TestKit(system) {{
      final Props props = Props.create(StockRegistryActor.class);
      final ActorRef subject = system.actorOf(props);
      final TestKit probe = new TestKit(system);
      
      within(duration("3 second"), () -> {
        subject.tell(
        ...
        subject.tell(new StockRegistryMessages.GetStocks(), getRef());
        expectMsgPF(duration("2 seconds"), "Should retrieve non empty stocks", (msg) -> {
          Stocks stocks = (Stocks) msg;
          Assert.assertEquals(stocks.getStocks().size(), 1);
          return null;
        });
